...
class IncomeManager {
  ...
  enableMultipleEarners() {
    ...
    if (firstEarner) {
      firstEarner.textContent = "Income Earner 1 (after tax)";
    }
  }

  disableMultipleEarners() {
    // Remove additional earners
    const earners = document.querySelectorAll(".income-card");
    ...
    const firstEarner = document.querySelector(".earner-title");
    if (firstEarner) {
      firstEarner.textContent = "Your Monthly Income (after tax)";
    }
  }
  ...
  createEarnerElement(earnerId) {
    ...
    newEarner.className = "income-card";
    ...
    newEarner.innerHTML = `
            <div class="income-header">
                <div class="earner-info">
                    ...
                    <span class="earner-title">Income Earner ${earnerId + 1} (after tax)</span>
                </div>
  ...
  getTotalIncome() {
    return Array.from(this.incomes.values()).reduce(
  ...
  getIncomeBreakdown() {
    ...
    this.incomes.forEach((income, earnerId) => {
      const earnerElement = document.querySelector(
        `[data-earner="${earnerId}"] .earner-title`,
      );
      const title = earnerElement
        ? earnerElement.textContent.trim()
        : `Earner ${earnerId + 1}`;
      breakdown.push({
        id: earnerId,
        title: title,
        amount: income,
        percentage:
          this.getTotalIncome() > 0
            ? (income / this.getTotalIncome()) * 100
            : 0,
      });
    });
    return breakdown;
  ...
  setIncome(earnerId, amount) {
    ...
    const input = document.querySelector(
      `[data-earner="${earnerId}"] .income-input`,
    );
    if (input) {
      input.value = amount;
    }
  ...
  validateIncomes() {
    ...
    this.incomes.forEach((income, earnerId) => {
      if (!isPositiveNumber(income)) {
        errors.push(
          `Income for earner ${earnerId + 1} must be a positive number`,
        );
      }
    });
